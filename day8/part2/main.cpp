...
long long matchBoxes(vector<Box>& boxes, vector<Distance>& distances, vector<Circuit>& circuits) {
  long long result;
  for (const Distance& distance : distances) {
    ...
    result = startBox.x * endBox.x;
    if (circuits.size() == 1 && circuits[0].getCircuitSize() == 1000) return result; 
    /*
    for (Circuit circuit : circuits) {
      cout << "[";
...
int main() {
  ...
  */
  ...
  cout << "Result: " << matchBoxes(boxes, distances, circuits);
